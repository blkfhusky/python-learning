...
def download_page(url):
    data = requests.get(url, headers=HEADERS).content  # 请求页面，获取要爬取得页面内容
    return data


def parse_html(html):
    soup = BeautifulSoup(html, "lxml")  # 使用bs解析获取的页面，测试是可以使用print soup.pretiffy()打印查看获得的页面
    ...
    for movie_li in movie_list_soup.find_all('li'):  # 遍历页面中有关的信息
        ...
        movie_link = detail.find('a').attrs['href']
        movie_link_list.append(movie_link)
    next_page = soup.find('span', attrs={'class': 'next'}).find('a')  # 找到下一页
    if next_page:
        return movie_name_list, movie_link_list, URL + next_page['href']  # 拼接下一页的url，继续爬取下一页
    return movie_name_list, movie_link_list, None  # 返回电影名称的list


def main():
    url = URL
    with codecs.open('movies', 'wb', encoding='utf-8') as fp:  # 打开文件，使用utf-8编码
        while url:
            html = download_page(url)  # 获取页面
            movies, links, url = parse_html(html)  # 分析页面获取信息
            items = dict(zip(movies, links))
            fp.write("\n".join(": ".join(_) for _ in items.items()))  # 将获得的信息写入文件
